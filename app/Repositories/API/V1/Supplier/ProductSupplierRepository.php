    ...
    try {
        ...
        if ($product) {
            ...
            $product->update([
                ...
                // Optional: Add any other fields you want to allow for updating
            ]);
            ...
            if (isset($credentials['product_images']) && is_array($credentials['product_images'])) {
                foreach ($credentials['product_images'] as $imageFile) {
                    ...
                    try {
                        ...
                        // Create a new image record associated with the product via the polymorphic relationship
                        $product->images()->create(['url' => $path]);
                    } catch (\Exception $e) {
                        Log::error('Failed to store image: ' . $e->getMessage());
                    }
            ...
            // Optionally, handle any other operations you need after updating
            return $product;
        }
